...
def load_model():
    ...
    latest_iteration.text("Loaded kaggle nesnet-like nn")
    bar.progress(1/4)
    ...
    spatial_map_layer = model.get_layer('add_11').output
    y_c = output_all_categories[0]
    grads_l = K.gradients(y_c, spatial_map_layer)[0]
    spatial_map_and_gradient_function = K.function([model.input], [spatial_map_layer, grads_l])
...
def get_heatmaps(image, model, funcs):
    ...
    spatial_map_all_dims, grads_val_all_dims = results
